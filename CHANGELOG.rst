...
- Add group-boundary AutoQuantize scoring and make it the default for gradient scoring. Attention projections are scored at their parent attention output and fused/shared MoE projections at their parent MLP output without grouping attention recipe decisions. Set ``score_boundary: local`` to restore leaf-output gradient scoring. KL-divergence retains local scoring.
